...
- Themes: a `[theme]` section in the config file (and `--theme NAME`) mapping the semantic
  styles (ok / warn / bad / alert / accent / dim / selected) to named ANSI colours, 256-colour
  indexes or RGB, with a few built-in presets and the terminal's own ANSI palette as the default.
  Today every style is a fixed named ANSI colour, so the terminal theme decides.
