...
- Out-of-range `-M`, `-c`, `-U`, `-Z`, `--rtt-thresholds` values are rejected instead of silently wrapping, and a negative value written as a separate word (`-M -1`, which C's getopt accepts) reaches that check instead of being read as an unknown flag (CR-10).
