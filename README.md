...
sockets are open).
    ...
    tar xzf mtr-rs-0.4.0-aarch64-macos.tar.gz && cd mtr-rs-0.4.0-aarch64-macos
...
each file, so a download can be checked against what the release workflow actually built:
    ...
    gh attestation verify mtr-rs-0.4.0-x86_64-linux.tar.gz --repo seitzbg/mtr-rs
...
## Development
    ...
    scripts/homebrew-formula.sh 0.4.0   # the Homebrew formula for a published release (seitzbg/homebrew-mtr-rs)
